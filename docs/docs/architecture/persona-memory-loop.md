---
sidebar_label: "Persona Memory Loop"
sidebar_position: 32
---

# Persona Memory Loop — Architecture View

> **Status:** Proposed (research stage). The formal decision lives in [ADR-032](../adr/adr-032-persona-memory-loop). This page is the architecture-flavoured view: how persona memory fits into the existing WI memory stack.
> **Date:** 2026-06-02
...
**Related**: [ADR-014](../adr/adr-014-self-learning-investigation-brain.md), [ADR-015](../adr/adr-015-mempalace-integration.md), [ADR-016](../adr/adr-016-second-brain-architecture.md), [ADR-024](../adr/adr-024-unified-brain.md), [ADR-030](../adr/adr-030-self-healing-bug-loop.md), [ADR-031](../adr/adr-031-per-bucket-model-effort-config.md)
...
This also means: an Open Question about per-rule write-lock concurrency is closed. SQLite's per-row locking is the lock.
...
- `src/services/persona/parsers/parse-eslint.ts` — loads resolved ESLint config; emits one `kind='rule'` palace drawer per active rule, scoped by config's `files`/`overrides` globs, severity from config. **Single mode in v1: full resolution only.** Runs only against repos in `PERSONA_TIER0_TRUSTED_REPOS` (default: WI repo) which by definition have `node_modules`. The "degraded mode" path the earlier draft contemplated for `./repos/example-service/` and `./repos/operations/` is **explicitly removed from v1** — see Hard scope rule above. Restoring Tier-0 ESLint coverage on customer repos is deferred to a future ADR (would need lint-only dep install OR a pre-resolved snapshot AND a strict-subset-safety story).
  - *Full resolution*: when `node_modules` is present (WI repo), follows `extends`/`plugins` chains.
  - *Degraded mode*: when `node_modules` is absent (`./repos/example-service/`, `./repos/operations/` per CLAUDE.md), reads only the `rules:` block at the top level of the config and tags each emitted drawer with `degraded_resolution=true`. Plugin-inherited rules are silently skipped — restoring full coverage on connected repos is deferred to a follow-up phase (would need lint-only deps install or a pre-resolved snapshot).
...
- Nightly Letta-style consolidation cron:
  - Cluster active lessons; surface clusters of size ≥ 3 as promotion candidates.
...
- [ADR-014: Self-learning investigation brain](../adr/adr-014-self-learning-investigation-brain.md) — same pattern, applied to investigations
- [ADR-015: MemPalace integration](../adr/adr-015-mempalace-integration.md) — palace primitives this ADR extends with a 7th wing
- [ADR-016: Second brain architecture](../adr/adr-016-second-brain-architecture.md) — recall-augmented chat foundation
- [ADR-024: Unified Brain](../adr/adr-024-unified-brain.md) — every consumer this ADR's persona memory flows through
- [ADR-030: Self-healing bug loop](../adr/adr-030-self-healing-bug-loop.md) — same `record_outcome → MemoryEnricher → palace` pattern, applied to bugs
- [ADR-031: Per-bucket model+effort config](../adr/adr-031-per-bucket-model-effort-config.md) — `agents` bucket reused for the cron
